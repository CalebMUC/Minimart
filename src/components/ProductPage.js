...
const ProductPage = () => {
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="hidden md:block relative group">
            <button 
              onClick={scrollLeft}
              className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white text-yellow-600 p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-200 shadow-lg hover:bg-yellow-50"
            >
            ...
            <div 
              ref={scrollRef}
              className="flex gap-4 overflow-x-auto py-2 scroll-smooth snap-x snap-mandatory no-scrollbar"
            >
            ...
            <button 
              onClick={scrollRight}
              className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white text-yellow-600 p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-200 shadow-lg hover:bg-yellow-50"
            >
          ...
          <div className="md:hidden grid grid-cols-2 gap-3">
            {items.slice(0, showAll ? items.length : 4).map((product, index) => (
              <ProductCard 
                key={index} 
                product={product} 
          ...
          {items.length > 4 && (
            <button
              ...
              className="mt-4 text-yellow-600 hover:text-yellow-700 flex items-center text-sm md:hidden"
            >
              {showAll ? (
                <>
                  <span>Show Less</span>
  ...
  <div className="bg-gray-50 min-h-screen p-4 md:p-8">
    ...
    {/* Main Content - Cart and Checkout Side by Side on Desktop */}
    <div className="flex flex-col lg:flex-row gap-6">
      {/* Cart Items Section */}
      ...
      {/* Checkout Section - On the same row as Cart on Desktop */}
